...
(BrowserStackBrowserCreator as any).prototype.setupBrowser = vi.fn().mockImplementation(options => {
...
describe('BrowserStack browserCreator ', () => {
  ...
  test('should try to reconnect in case of a full BrowserStack queue', async () => {
    ...
    expect((browserCreator as any).setupBrowser.mock.calls.length).toBe(0);
    await expect(browserCreator.getBrowser({ makeQueueFullErrorForTesting: true } as any)).rejects.toThrow(
      'All parallel tests are currently in use'
    );
    expect((browserCreator as any).setupBrowser.mock.calls.length).toBe(11);
  });
  ...
  test('should not try to reconnect if any other error than a full BrowserStack queue gets thrown', async () => {
    ...
    expect((browserCreator as any).setupBrowser.mock.calls.length).toBe(1);
  });
